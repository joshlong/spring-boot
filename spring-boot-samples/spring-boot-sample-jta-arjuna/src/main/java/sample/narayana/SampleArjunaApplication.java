...
/**
 * Arjuna is the name of the JTA sub-project in the larger <A href="http://narayana.jboss.org/">Narayana</A>  project
 * which aims to support all things transactional. It can be quite confusing to figure out what libraries to use
 * to make this work in a standalone fashion. The {@code org.springframework.boot:spring-boot-starter-jta-arjuna}
 * starter is invaluable here.
 *
 * @author Josh Long
 */
@Configuration
...
public class SampleArjunaApplication {
    ...
    /**
     * Registers a {@link javax.sql.DataSource} using the {@link org.springframework.boot.autoconfigure.jta.arjuna.ArjunaXaDataSourceFactoryBean}
     * that in turn adapts Arjuna's XA-aware {@link java.sql.Driver} into a {@link javax.sql.DataSource}.
     */
    @Bean
    ...
    private XADataSource targetDataSource(String host, String db, String username, String pw) {
     ...
     * the convenient XA-aware connection factory ({@link org.apache.activemq.jms.pool.XaPooledConnectionFactory XaPooledConnectionFactory})
     * from the <A href="http://activemq.apache.org/">ActiveMQ</A> project itself.
     * <p>
     * This also awkwardly requires a reference to the JTA {@link javax.transaction.TransactionManager}.
     * Here we use the injected {@link org.springframework.transaction.jta.JtaTransactionManager}
     * to obtain it, though we could have as easily called a JTA-API specific factory method like
